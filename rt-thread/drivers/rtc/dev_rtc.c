...
rt_err_t set_date(rt_uint32_t year, rt_uint32_t month, rt_uint32_t day)
{
    ...
    ret = rt_device_control(
        _rtc_device, RT_DEVICE_CTRL_RTC_GET_TIME, &old_timestamp);
    if (ret != RT_EOK)
    {
        return ret;
    }
...
rt_err_t set_timestamp(time_t timestamp)
{
    ...
    return rt_device_control(
        _rtc_device, RT_DEVICE_CTRL_RTC_SET_TIME, &timestamp);
}
...
rt_err_t get_timestamp(time_t *timestamp)
{
    ...
    return rt_device_control(
        _rtc_device, RT_DEVICE_CTRL_RTC_GET_TIME, timestamp);
}
...
static void date(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        rt_kprintf("timezone: UTC%c%02d:%02d:%02d\n",
            ...
            abs_tz_offset_sec % 3600U / 60U, abs_tz_offset_sec % 3600U % 60U);
    }
    else if (argc >= 7)
    {
        /* set time and date */
    ...
    {
        rt_kprintf(
            "please input: date [year month day hour min sec] or date\n");
