...
static rt_ssize_t rt_hwtimer_read(
    struct rt_device *dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    rt_hwtimer_t *timer;
    rt_hwtimerval_t tv;
    rt_uint32_t cnt;
...
static rt_ssize_t rt_hwtimer_write(
    struct rt_device *dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    rt_base_t level;
    rt_uint32_t t;
...
void rt_device_hwtimer_isr(rt_hwtimer_t *timer)
{
    ...
    if (timer->cycles == 0)
    {
        ...
        {
            timer->parent.rx_indicate(
                &timer->parent, sizeof(struct rt_hwtimerval));
        }
    }
    else
    {
...
const static struct rt_device_ops hwtimer_ops = { rt_hwtimer_init,
    rt_hwtimer_open, rt_hwtimer_close, rt_hwtimer_read, rt_hwtimer_write,
    rt_hwtimer_control };
#endif
    ...
    rt_hwtimer_t *timer, const char *name, void *user_data)
{
    ...
    return rt_device_register(
        device, name, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_STANDALONE);
}
