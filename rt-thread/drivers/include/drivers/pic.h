...
struct rt_pic_ops
{
    ...
    rt_err_t (*irq_set_affinity)(
        struct rt_pic_irq *pirq, rt_bitmap_t *affinity);
    ...
    void (*irq_compose_msi_msg)(
        struct rt_pic_irq *pirq, struct rt_pci_msi_msg *msg);
    void (*irq_write_msi_msg)(
        struct rt_pic_irq *pirq, struct rt_pci_msi_msg *msg);
...
#define RT_IRQ_STATE_MASKED  2
    rt_err_t (*irq_set_state)(
        struct rt_pic *pic, int hwirq, int type, rt_bool_t state);
    rt_err_t (*irq_get_state)(
        struct rt_pic *pic, int hwirq, int type, rt_bool_t *out_state);

    int (*irq_map)(struct rt_pic *pic, int hwirq, rt_uint32_t mode);
    rt_err_t (*irq_parse)(struct rt_pic *pic,
...
#define RT_IRQ_MODE_EDGE_BOTH                                                  \
    (RT_IRQ_MODE_EDGE_FALLING | RT_IRQ_MODE_EDGE_RISING)
...
typedef rt_err_t (*__kpi_rt_pic_attach_irq)(
    int irq, rt_isr_handler_t handler, void *uid, const char *name, int flags);
typedef rt_err_t (*__kpi_rt_pic_detach_irq)(int irq, void *uid);
...
typedef rt_err_t (*__kpi_rt_pic_irq_init)(void);
typedef rt_err_t (*__kpi_rt_pic_irq_finit)(void);
typedef void (*__kpi_rt_pic_irq_enable)(int irq);
typedef void (*__kpi_rt_pic_irq_disable)(int irq);
typedef void (*__kpi_rt_pic_irq_ack)(int irq);
typedef void (*__kpi_rt_pic_irq_mask)(int irq);
typedef void (*__kpi_rt_pic_irq_unmask)(int irq);
typedef void (*__kpi_rt_pic_irq_eoi)(int irq);
typedef rt_err_t (*__kpi_rt_pic_irq_set_priority)(
    int irq, rt_uint32_t priority);
typedef rt_uint32_t (*__kpi_rt_pic_irq_get_priority)(int irq);
typedef rt_err_t (*__kpi_rt_pic_irq_set_affinity)(
    int irq, rt_bitmap_t *affinity);
typedef rt_err_t (*__kpi_rt_pic_irq_get_affinity)(
    int irq, rt_bitmap_t *out_affinity);
typedef rt_err_t (*__kpi_rt_pic_irq_set_triger_mode)(int irq, rt_uint32_t mode);
typedef rt_uint32_t (*__kpi_rt_pic_irq_get_triger_mode)(int irq);
typedef void (*__kpi_rt_pic_irq_send_ipi)(int irq, rt_bitmap_t *cpumask);

#if 0
rt_err_t rt_pic_irq_set_state_raw(
    struct rt_pic *pic, int hwirq, int type, rt_bool_t state);
rt_err_t rt_pic_irq_get_state_raw(
    struct rt_pic *pic, int hwirq, int type, rt_bool_t *out_state);
...
rt_err_t rt_pic_irq_parent_set_priority(
    struct rt_pic_irq *pirq, rt_uint32_t priority);
rt_err_t rt_pic_irq_parent_set_affinity(
    struct rt_pic_irq *pirq, rt_bitmap_t *affinity);
rt_err_t rt_pic_irq_parent_set_triger_mode(
    struct rt_pic_irq *pirq, rt_uint32_t mode);
...
KPI_EXTERN(rt_pic_attach_irq);
KPI_EXTERN(rt_pic_detach_irq);
KPI_EXTERN(rt_pic_irq_enable);
KPI_EXTERN(rt_pic_irq_disable);
KPI_EXTERN(rt_pic_irq_ack);
KPI_EXTERN(rt_pic_irq_mask);
KPI_EXTERN(rt_pic_irq_unmask);
KPI_EXTERN(rt_pic_irq_eoi);
KPI_EXTERN(rt_pic_irq_set_priority);
KPI_EXTERN(rt_pic_irq_get_priority);
KPI_EXTERN(rt_pic_irq_set_affinity);
KPI_EXTERN(rt_pic_irq_get_affinity);
KPI_EXTERN(rt_pic_irq_set_triger_mode);
KPI_EXTERN(rt_pic_irq_get_triger_mode);
KPI_EXTERN(rt_pic_irq_send_ipi);
