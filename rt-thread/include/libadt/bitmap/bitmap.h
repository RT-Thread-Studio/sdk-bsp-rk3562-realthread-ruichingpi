...
#define RT_BITMAP_LEN(bits)                                                    \
    (((bits) + (RT_BITMAP_BITS_MIN) - 1) / (RT_BITMAP_BITS_MIN))
...
{
    bitmap[bit / RT_BITMAP_BITS_MIN] |=
        (1UL << (bit & (RT_BITMAP_BITS_MIN - 1)));
}

rt_inline rt_bool_t rt_bitmap_test_bit(rt_bitmap_t *bitmap, rt_uint32_t bit)
{
    return !!(bitmap[bit / RT_BITMAP_BITS_MIN] &
        (1UL << (bit & (RT_BITMAP_BITS_MIN - 1))));
}
...
rt_inline rt_size_t rt_bitmap_next_set_bit(
    rt_bitmap_t *bitmap, rt_size_t start, rt_size_t limit)
{
    rt_size_t bit;
...
rt_inline rt_size_t rt_bitmap_next_clear_bit(
    rt_bitmap_t *bitmap, rt_size_t start, rt_size_t limit)
{
    rt_size_t bit;
