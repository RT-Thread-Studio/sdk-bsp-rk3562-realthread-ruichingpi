...
class NCNN_EXPORT VkCompute
{
  ...
  public:
    ...
    void record_clone(
        const VkImageMat &src, VkImageMat &dst, const Option &opt);
    ...
    int get_query_pool_results(uint32_t first_query,
...
class NCNN_EXPORT VkTransfer
{
  ...
  public:
    void record_upload(
        const Mat &src, VkMat &dst, const Option &opt, bool flatten = true);
