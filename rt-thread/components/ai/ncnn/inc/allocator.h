...
#define NCNN_XADD(addr, delta)                                                 \
    (int)__atomic_fetch_add(                                                   \
        (unsigned *)(addr), (unsigned)(delta), __ATOMIC_ACQ_REL)
#else
#define NCNN_XADD(addr, delta)                                                 \
...
class NCNN_EXPORT VkAllocator
{
  public:
    ...
    virtual VkImageMemory *fastMalloc(
        int w, int h, int c, size_t elemsize, int elempack) = 0;
  ...
  protected:
    ...
    VkImage create_image(int width,
        int height,
        int depth,
        VkFormat format,
