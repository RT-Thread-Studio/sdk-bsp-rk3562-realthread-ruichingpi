...
struct NCNN_EXPORT Mat
{
    ...
    void create(int _rows, int _cols, int flags)
    {
        ...
        if (total() > 0)
        {
            ...
            data =
                (uchar *)ncnn::fastMalloc(totalsize + (int)sizeof(*refcount));
...
NCNN_EXPORT Mat imdecode(
    const std::vector<uchar> &buf, int flags = IMREAD_COLOR);
...
NCNN_EXPORT bool imwrite(const std::string &path,
    const Mat &m,
...
#if NCNN_PIXEL
NCNN_EXPORT void resize(const Mat &src,
    Mat &dst,
    const Size &size,
    float sw = 0.f,
    float sh = 0.f,
...
NCNN_EXPORT void rectangle(
    Mat &img, Point pt1, Point pt2, const Scalar &color, int thickness = 1);

NCNN_EXPORT void rectangle(
    Mat &img, Rect rec, const Scalar &color, int thickness = 1);

NCNN_EXPORT void circle(
    Mat &img, Point center, int radius, const Scalar &color, int thickness = 1);

NCNN_EXPORT void line(
    Mat &img, Point p0, Point p1, const Scalar &color, int thickness = 1);
...
NCNN_EXPORT void putText(Mat &img,
...
NCNN_EXPORT Size getTextSize(const std::string &text,
    int fontFace,
    double fontScale,
    int thickness,
    int *baseLine);
