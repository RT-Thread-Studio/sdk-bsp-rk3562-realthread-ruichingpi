...
class NCNN_EXPORT PipelineCache
{
  public:
    ...
    int get_pipeline(const uint32_t *spv_data,
    ...
    int get_pipeline(int shader_type_index,
        const Option &opt,
        const std::vector<vk_specialization_type> &specializations,
        uint32_t local_size_x,
        uint32_t local_size_y,
        ...
        ShaderInfo &shader_info) const;
  protected:
    int create_shader_module(int shader_type_index,
        const Option &opt,
        uint32_t local_size_x,
        uint32_t local_size_y,
        uint32_t local_size_z,
    ...
    int new_pipeline(VkShaderModule shader_module,
