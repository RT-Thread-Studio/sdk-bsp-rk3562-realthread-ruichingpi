...
TIMEVAL getElapsedTime(void)
{
    ...
    return (val.sec - last_timer_val.sec) * 1000000 +
        (val.usec - last_timer_val.usec);
}
...
static void canopen_timer_thread_entry(void *parameter)
{
    while (1)
    {
        ...
        EnterMutex();
        rt_size_t read_size = rt_device_read(
            canfstvl_timer_dev, 0, &last_timer_val, sizeof(last_timer_val));
        if (read_size == 0)
        {
...
void initTimer(void)
{
    ...
    tid = rt_thread_create("cf_timer", canopen_timer_thread_entry, RT_NULL,
        40960, CANFESTIVAL_TIMER_THREAD_PRIO, 20);
    if (tid != RT_NULL)
