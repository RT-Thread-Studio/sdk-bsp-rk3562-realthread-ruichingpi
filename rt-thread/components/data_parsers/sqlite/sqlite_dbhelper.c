...
int db_connect(char *name)
{
    int32_t len = 0;
    rt_mutex_take(db_mutex_lock, RT_WAITING_FOREVER);
    len = rt_strnlen(name, COMP_USING_SQLITE_DB_NAME_MAX_LEN + 1);
    if (len >= COMP_USING_SQLITE_DB_NAME_MAX_LEN + 1)
    {
        LOG_E("the database name '(%s)' lengh is too long(max:%d).", name, COMP_USING_SQLITE_DB_NAME_MAX_LEN);
        rt_mutex_release(db_mutex_lock);
        return -RT_ERROR;
    }
...
{
    static char name[COMP_USING_SQLITE_DB_NAME_MAX_LEN + 1];
