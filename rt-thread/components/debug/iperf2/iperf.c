...
int iperf(int argc, char **argv)
{
    ...
    if (argv[1] != RT_NULL && strcmp(argv[1], "-u") == 0)
    {
        index = 2;
        use_udp = 1;
    }
    if (argv[index] != RT_NULL && strcmp(argv[index], "-h") == 0) goto __usage;
    else if (argv[index] != RT_NULL && strcmp(argv[index], "--stop") == 0)
    {
        /* stop iperf */
        param.mode = IPERF_MODE_STOP;
        return 0;
    }
    else if (argv[index] != RT_NULL && strcmp(argv[index], "-s") == 0)
    {
        ...
        if (argc >= 4)
        {
            if (argv[index + 1] != RT_NULL && strcmp(argv[index + 1], "-p") == 0)
            {
                if (argv[index + 2] == RT_NULL)
                {
                    goto __usage;
                }
    ...
    else if (argv[index] != RT_NULL && strcmp(argv[index], "-c") == 0)
    {
        mode = IPERF_MODE_CLIENT; /* client mode */
        if (argc < 3) goto __usage;

        if (argv[index + 1] == RT_NULL)
        {
            goto __usage;
        }
        ...
        {
            /* iperf -c host -p port */
            if (argv[index + 2] != RT_NULL && strcmp(argv[index + 2], "-p") == 0)
            {
                if (argv[index + 3] == RT_NULL)
                {
                    goto __usage;
                }
    ...
    if (argc >= 7)
    {
        if(argv[argc - 2] != RT_NULL && strcmp(argv[argc - 2], "-m") == 0)
        {
            if (argv[argc - 1] == RT_NULL)
            {
                goto __usage;
            }
    ...
    if (param.mode == IPERF_MODE_STOP)
    {
        ...
        for (i = 0; i < numtid; i++)
        {
            ...
            tid = rt_thread_create(tid_name, function, RT_NULL, IPERF_THREAD_STACK_SIZE, 19, 100);
