...
#define MSH_FUNCTION_EXPORT_CMD(name, cmd, desc, opt)                          \
    ...
    const struct finsh_syscall __fsym_##cmd = { __fsym_##cmd##_name,           \
        FINSH_DESC(cmd, desc) FINSH_COND(opt)(syscall_func) & name };
...
#define MSH_FUNCTION_EXPORT_CMD(name, cmd, desc, opt)                          \
    ...
    rt_used const struct finsh_syscall __fsym_##cmd                            \
        rt_section("FSymTab") = { __fsym_##cmd##_name,                         \
            FINSH_DESC(cmd, desc) FINSH_COND(opt)(syscall_func) & name };
...
#define MSH_CMD_EXPORT(...)                                                    \
    __MSH_GET_MACRO(__VA_ARGS__, _MSH_FUNCTION_CMD2_OPT, _MSH_FUNCTION_CMD2)(  \
        __VA_ARGS__)
...
#define CMD_OPTIONS_STATEMENT(command)                                         \
    static struct msh_cmd_opt command##_msh_options[];
...
#define MSH_OPT_ID_GET(fun)                                                    \
    msh_cmd_opt_id_get(argc, argv, (void *)fun##_msh_options)
